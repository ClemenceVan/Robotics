...
void cox_linefit(/*std::vector<float> angle, std::vector<float> distance, std::vector<float> position*/) {
    ...
    std::vector<double> assigned_line_index(positionMatrix.rows(),0);
    std::vector<double> squared_dists(positionMatrix.rows(),0);
    std::vector<double> all_yi(positionMatrix.rows(),0);
    ...
    for(int i = 0; i < max_iterations; i++) {
        ...
        //std::cout << all_vi << std::endl;
        //std::cout << all_vi.rows() << "size all_vi" << std::endl;
        for(int j = 0; j < all_vi.rows(); j++) //loop throguh all lidar points
        {
            ...
            for(int k = 0; k < line_model.rows(); k++) // loop throguh all lines
            {
       ...
       std::cout << assigned_line_index.size() << std::endl;
       std::cout << unit_vectors.rows() << std::endl;
        // for (int a = 0; a < assigned_line_index.size(); a++)
