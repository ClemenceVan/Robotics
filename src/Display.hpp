...
class Display {
    ...
    ftxui::Component lidar = Renderer([this] {
        ...
        return vbox({
            window(text("Position:"), canvas(std::move(c)) | border)
        }) | yframe | flex;
    ...
    Display(std::pair<int, int> canvasSize, Arena arena) {
        ...
        Eigen::MatrixXd line_model = arena.getLineModel();
    ...
    void addFigure(Eigen::MatrixXd positions, ftxui::Color color) {
        ...
        //     c.DrawPoint(CenterPointX - positions(i, 0) + ArenaWidth/2 - (int)OriginX, CenterPointY - positions(i, 1) + ArenaHeight/2 - (int)OriginY, true, color);
    }
